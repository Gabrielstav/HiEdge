...
from src.setup.data_structures import BedpeOutput, BlacklistOutput, CytobandOutput, StatInput
...
class ResolveBedpeInput:
    """
    Resolves what BEDPE data class to pass to processing for statistical modeling based on config file (no filtering, blacklist only, cytoband only, blacklist and cytoband)
    ...
    def resolve_input(self):
        ...
        elif isinstance(self.data_output, BlacklistOutput) and self.config.config_data.pipeline_settings.filter_blacklist:
            return self.data_output
        elif isinstance(self.data_output, BedpeOutput):
            return self.data_output
        else:
...
class CalculateMidpoints:
    ...
    def process_data(self):
        # Calculate midpoints for each bin using vectorized operations
